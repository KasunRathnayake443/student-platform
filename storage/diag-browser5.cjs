...
(async () => {
    ...
    await send('Runtime.evaluate', { expression: document.documentElement.outerHTML.substring(0, 200).length ? true : true, returnByValue: true });
    const dumpHtml = async () => { const r = await send('Runtime.evaluate', { expression: 'document.querySelector(\\'#teacher-app\\').outerHTML.substring(0, 400)', returnByValue: true }); console.log('APP HEAD:', r.result?.value); const r2 = await send('Runtime.evaluate', { expression: 'document.body.innerHTML.indexOf(\\'wire:id\\')', returnByValue: true }); console.log('wire:id first index in body:', r2.result?.value); const r3 = await send('Runtime.evaluate', { expression: 'document.body.innerHTML.substring(Math.max(0,document.body.innerHTML.indexOf(\\'wire:id\\')-300), document.body.innerHTML.indexOf(\\'wire:id\\')+100)', returnByValue: true }); console.log('WIRE ID CONTEXT:', r3.result?.value); };
    await dumpHtml();
